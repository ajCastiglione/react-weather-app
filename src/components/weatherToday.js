...
class WeatherToday extends Component {
  state = {
    today:
      sessionStorage.today !== undefined
        ? JSON.parse(sessionStorage.today)
        : [],
    fetchForcast: false,
    lat:
      localStorage.coords !== undefined
        ? JSON.parse(localStorage.coords)[0]
        : this.props.lat,
    long:
      localStorage.coords !== undefined
        ? JSON.parse(localStorage.coords)[1]
        : this.props.long
  };
  ...
  getToday = () => {
    $.ajax({
      url: `https://api.darksky.net/forecast/${key}/${this.props.lat},${
        this.props.long
      ...
      success: function(r) {
        ...
        let unixConvert = new Date(today.time * 1000),
          day = unixConvert.getDay(),
            ...
            <div className="today-content col-xs-12">
              <div className="inner-today-content">
                <div className="weather-data">
                  <p className="weather-single-temp">
                    {Math.round(today[0].temp)} <sup>&deg;F</sup>
                  </p>
                  <p className="weather-single-summary">{today[0].summary}</p>
                  <div className="weather-single-humidity">
                    <p>humidity</p>
                    <p>{Math.round(today[0].humidity * 100)}%</p>
                  </div>
